...
class llmrec_model(nn.Module):
    ...
    def make_interact_text(self, interact_ids, interact_max_num, user):
        ...
        def format_time_diff(diff_ms):
            """Convert millisecond difference to human readable format"""
            diff_seconds = diff_ms / 1000

            if diff_seconds < 60:
                return f"{int(diff_seconds)} seconds"
        ...
        for idx, (title, item_id) in enumerate(zip(items_to_process, ids_to_process)):
            if idx == 0:
                # First item: "User A watched 'Title'"
                interact_text.append(f"User A watched {title}[HistoryEmb]")
            else:
                # Subsequent items: "then after X time watched 'Title'"
                time_diff = timestamps[idx] - timestamps[idx-1]
                time_diff_str = format_time_diff(time_diff)
                interact_text.append(f"then after {time_diff_str} watched {title}[HistoryEmb]")

        # Join with ", " to create proper sentences
