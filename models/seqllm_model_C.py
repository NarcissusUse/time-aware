...
class llmrec_model(nn.Module):
    ...
    def make_interact_text(self, interact_ids, interact_max_num, user):
        ...
        for idx, (title, time_str) in enumerate(zip(items_to_process, times)):
            if idx == 0:
                # First item: "User A watched 'Title' on Jan 1, 2022 at 9:23 AM"
                interact_text.append(f"User A watched {title} on {time_str}[HistoryEmb]")
            else:
                # Subsequent items: "Then watched 'Title' on Jan 1, 2022 at 9:23 AM"
                interact_text.append(f"Then watched {title} on {time_str}[HistoryEmb]")
