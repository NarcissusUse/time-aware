...
class llmrec_model(nn.Module):
    ...
    def make_interact_text(self, interact_ids, interact_max_num, user):
        ...
        # Calculate session start time (first item's timestamp)
        session_start_ms = timestamps[0] if timestamps else 0
    
        def format_session_time(timestamp_ms, session_start_ms):
            """Convert timestamp to session-relative seconds"""
            diff_ms = timestamp_ms - session_start_ms
            diff_seconds = diff_ms / 1000
            return int(diff_seconds)
    
        # Build the session-relative format
        session_parts = []
        for idx, (title, item_id) in enumerate(zip(items_to_process, ids_to_process)):
            relative_seconds = format_session_time(timestamps[idx], session_start_ms)
            
            if idx == 0:
                # First item: "t=0s: Movie A (session start)"
                session_parts.append(f"t={relative_seconds}s: {title}[HistoryEmb] (session start)")
            else:
                # Subsequent items: "t=Xs: Movie B"
                session_parts.append(f"t={relative_seconds}s: {title}[HistoryEmb]")
    
        # Join with " → " to create the session format
